...
/* errTypes where the program is exited are for debugging.
They don't occur when using only URF.h interface */
void URF_warn(char errType)
{
  ...
  {
    fprintf(stderr,
     "WARNING: In URF finder: Tried to add an edge with atoms not in range.\n");
  }
  else if(errType == 'v')
  {
    fprintf(stderr,
      "ERROR: Tried to initialize a graph with negative number of vertices.\n");
  }
  else if(errType == 'f')
  {
