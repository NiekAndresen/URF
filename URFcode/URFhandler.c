...
/* similar to the function "List_Paths" from Vismara*/
  ...
  array[b] = 1;
  /*for each vertex URF_adjacent to b in U_a*/
...
void URF_listPaths(int r, int p, char ***paths, int *currPath, int *alloced,
                    char mode, URF_graph *gra, sPathInfo *spi)
{
  ...
  if(spi->dPaths[r]->degree[p] > 1)/*split*/
  {
    ...
    /*for each vertex URF_adjacent to p in U_r*/
    for(i=0; i<spi->dPaths[r]->degree[p]; ++i)
    {
