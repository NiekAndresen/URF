...
/*TODO something to write: This could contain a flag storing if the URFs have been calculated before for this graph. Every function called would first check if this flag is set and call 'calculateURFs()' if it is not. This would make the 'calculateURFs()' call unnecessary. I won't do this because it would require the user to initialize his URF_data ** to NULL before calling the function for the first time (or get "used uninitialized.."). Also he would have to have a URF_data ** instead of a URF_data * which requires dereferencing one more time.*/
typedef struct{
  URF_graph *graph;
